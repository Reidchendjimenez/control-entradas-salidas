...
/// que sirve `tool/server.py` (misma app, sin CORS). En nativo (Windows/
/// Android) se intenta primero el sitio oficial directo con SSL flexible.
/// Respaldo: bcv.today.
/// El valor guardado vive en `pos_settings` vía `setTasaCambio`.
class TasaBcvService {
  TasaBcvService({http.Client? client}) : _client = client ?? _createClient();
  ...
  /// Crea un cliente HTTP con SSL flexible en nativo (certificados del BCV
  /// a veces no verifican correctamente). En web usa el cliente estándar.
  ...
  Future<double> obtenerTasaBcv({int timeout = 15}) async {
    // En web: proxy → sitio oficial → bcv.today
    // En nativo: sitio oficial → bcv.today (no hay proxy)
    if (kIsWeb) {
      try {
    ...
    try {
      ...
      _ultimoError = '${_ultimoError != null ? '$_ultimoError; ' : ''}sitio oficial: $e';
    }
    try {
