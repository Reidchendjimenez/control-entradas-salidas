...
/// Descriptores de tablas POS para Realtime (usa la misma interfaz que
/// el sync general).
const List<SyncTableDescriptor> posTableDescriptors = [
  SyncTableDescriptor(serverTable: 'platos_categorias', localTable: 'platos_categorias', dedupeKey: 'nombre', incrementalColumn: 'updated_at'),
  SyncTableDescriptor(serverTable: 'platos', localTable: 'platos', incrementalColumn: 'updated_at'),
  SyncTableDescriptor(serverTable: 'plato_ingredientes', localTable: 'plato_ingredientes'),
  SyncTableDescriptor(serverTable: 'plato_contornos', localTable: 'plato_contornos'),
  SyncTableDescriptor(serverTable: 'pos_categorias', localTable: 'pos_categorias', dedupeKey: 'nombre', incrementalColumn: 'updated_at'),
  SyncTableDescriptor(serverTable: 'pos_mesas', localTable: 'pos_mesas', incrementalColumn: 'updated_at'),
  SyncTableDescriptor(serverTable: 'pos_habitaciones', localTable: 'pos_habitaciones', incrementalColumn: 'updated_at'),
  SyncTableDescriptor(serverTable: 'pos_usuarios', localTable: 'pos_usuarios', incrementalColumn: 'updated_at'),
  SyncTableDescriptor(serverTable: 'pos_settings', localTable: 'pos_settings'),
  SyncTableDescriptor(serverTable: 'pos_sesiones', localTable: 'pos_sesiones'),
  SyncTableDescriptor(serverTable: 'pos_comandas', localTable: 'pos_comandas'),
  SyncTableDescriptor(serverTable: 'pos_ventas', localTable: 'pos_ventas'),
];
...
class PosSyncEngine {
  ...
  late final RealtimeSource? _realtime;
  ...
  late final SyncEngine _general = SyncEngine(
    db: _db,
    client: client,
    realtime: _realtime,
  )..onProgress = _log;
  ...
  // ---------------------------------------------------------------------
  // Realtime: escuchar cambios POS en tiempo real
  // ---------------------------------------------------------------------

  /// Suscripciones Realtime de tablas POS.
  final List<StreamSubscription<RealtimeEvent>> _realtimeSubs = [];

  /// Inicia suscripciones Realtime para las tablas POS.
  ///
  /// Permite que cambios de otros dispositivos POS (comandas, ventas, etc.)
  /// se reflejen localmente sin esperar el timer de polling.
  void startRealtime() {
    if (_realtime == null) return;
    for (final tableName in posTableNames) {
      final sub = _realtime!.watchTable(tableName).listen(
        (event) => _handleRealtimeEvent(event),
      );
      _realtimeSubs.add(sub);
    }
    // También escuchar tablas del sync general (categorias, productos, etc.)
    // que son relevantes para el POS.
    _general.startRealtime();
    _log('Realtime POS activo para ${posTableNames.length} tablas');
  }
  ...
  void stopRealtime() {
    ...
    _realtimeSubs.clear();
    _general.stopRealtime();
  }

  /// Procesa un evento Realtime de tabla POS.
  Future<void> _handleRealtimeEvent(RealtimeEvent event) async {
    try {
      ...
      // INSERT / UPDATE: upsert genérico basado en el nombre de tabla.
      // Reutilizamos la lógica de descarga del sync engine.
      final desc = posTableDescriptors.firstWhere(
        (d) => d.serverTable == event.table,
        orElse: () => SyncTableDescriptor(
          serverTable: event.table,
          localTable: event.table,
        ),
      );
      await _general.upsertLocalBatch(desc, [event.row]);
    } catch (e) {
      _log('Error en realtime POS ${event.table}: $e');
    }
  }
