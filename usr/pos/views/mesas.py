...
class MesasView(ft.Container):
    ...
    def _build_ui(self):
        top_bar = self._build_top_bar(titulo="Mesas", on_back=self.on_back)
    ...
    def _load_mesas(self):
        ...
        if not mesas:
            self.grid.controls.append(ft.Container(
                content=ft.Column([
                    ft.Icon(ft.Icons.TABLE_RESTAURANT_ROUNDED, size=80, color="#9E9E9E"),
            ...
            for m in mesas:
                self.grid.controls.append(self._build_card(m))
        if self.page:
            self.update()

    def _build_card(self, mesa: dict):
        ...
        card = ft.Container(
            ...
            content=ft.Column(
                ...
                controls=[
                    ...
                    ft.Text(
                        info.upper() if info else f"MESA {numero}",
        ...
        card.on_click = lambda _, m=mesa: self._go_comanda(m)
        return card
    ...
    def _go_comanda(self, mesa):
        if self.page:
            from usr.pos.views.comanda_view import ComandaPedidoView
            v = ComandaPedidoView(
                usuario=self.usuario, sesion_id=self.sesion_id,
                mesa=mesa, on_logout=self.on_logout,
                on_back=lambda: self._volver_mesas(),
            )
            self.page.clean()
            self.page.add(v)
            self.page.update()

    def _volver_mesas(self):
        if self.page:
